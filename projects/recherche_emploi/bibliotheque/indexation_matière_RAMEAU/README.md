# Indexation matière RAMEAU

Ce dossier est dédié à l’indexation matière avec RAMEAU : comprendre les vedettes, les subdivisions et le processus d’indexation professionnelle.

## Contenu du dossier

- `fiche_memo.md` : fiche A4 synthétique sur la structure d’une vedette, les types de vedettes et les erreurs fréquentes.
- `doc_expert.md` : dossier complet expliquant la logique RAMEAU, les étapes d’indexation et les formulations à utiliser en entretien.
- `exercices.md` : 30 exercices d’indexation (sujets variés) à traiter comme en situation de travail.
- `exercices_corriges.md` : corrections détaillées des exercices avec propositions de vedettes et subdivisions.

## Comment l’utiliser

1. Lire `fiche_memo.md` pour revoir la structure RAMEAU.
2. Approfondir avec `doc_expert.md` (concepts et méthode).
3. Faire les exercices dans `exercices.md` sans regarder les réponses.
4. Comparer avec `exercices_corriges.md` et noter les vedettes/subdivisions à retenir.
